...
1. Modular Binomal Theorem을 이용해 식을 정리
    ...
    $c_1 ≡ 2p^{e1} + 3q^{e1}\ (mod\ N) \\ c_2 ≡ 5p^{e2} + 7q^{e2}\ (mod\ N)$
    
2. 연립 합동식 풀이
    1. 지수를 맞추기 위해 $e^1, e^2$ 제곱
        
        ${c_1}^{e_2} ≡ 2p^{e_1e_2} + 3q^{e_1e_2}\ (mod\ N)\\{c_2}^{e_1} ≡ 5p^{e_1e_2} + 7q^{e_1e_2}\ (mod\ N)$
        
    2. 계수 맞춰주기 ($p$ 기준)
        
        ${c_1}^{e_2} * (5^{e_1e_2}) ≡ 10p^{e_1e_2} + 15q^{e_1e_2}\ (mod\ N)\\{c_2}^{e_1} * (2^{e_1e_2}) ≡ 10p^{e_1e_2} + 14q^{e_1e_2}\ (mod\ N)$
